"""Train the baseline tract-level random forest classifier."""
...
METRICS_PATH = Path("ml/outputs/random_forest_metrics.json")
IMPORTANCE_PATH = Path("ml/outputs/random_forest_feature_importance.csv")
PREDICTIONS_PATH = Path("ml/outputs/random_forest_predictions.csv")
...
def train_model(
    ...
    """Train an imputed random forest comparison baseline."""
...
def export_outputs(
    ...
    """Write metrics, feature importance, predictions, and the model artifact."""
    output_paths = {
        "metrics": repo_root / METRICS_PATH,
        "feature_importance": repo_root / IMPORTANCE_PATH,
    ...
    importance_frame.to_csv(output_paths["feature_importance"], index=False)
...
def main() -> None:
    ...
    print("Trained baseline random forest for tract-level arrest activity.")
    ...
    print(
        ...
        f"{output_paths['feature_importance'].relative_to(repo_root)}"
    )
